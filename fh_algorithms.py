...
import math
import cv2
def fh_check_take_back(tb_list, tb_unnorm, feedback):
    ...
    if right_index and left_hip:
        ...
        if distance > -100:
            ...
            feedback['take_back']['Hand Position'] = 'Bring your racquet back farther'
    ...
    if right_foot:
        ...
        if distance_rf < 100:
            ...
            feedback['take_back']['Right Foot'] = 'Bring the right foot farther out'
    ...
    if left_foot:
        ...
        if distance < 160:
            ...
            feedback['take_back']['Left Foot'] = 'Bring the left foot farther out'
    ...
    if right_hip and left_hip:
        ...
        if distance_hip > 150:
            ...
            feedback['take_back']['Hips'] = 'Looks good!'
        else:
...
def fh_check_contact_point(cp_list, c_unnorm, feedback):
    ...
    if right_hip and left_hip:
        ...
        if distance_hip > 100:
            ...
            feedback['contact_point']['Hips'] = 'Ensure your hips are aligned towards the target'
    ...
    # Check angle between shoulder, hand, and elbow
    if right_shoulder and right_elbow and right_index:
        ...
        if angle_deg < 140 or angle_deg > 220:  # Adjust threshold as needed
            ...
            feedback['contact_point']['Elbow Angle'] = 'Straighten your arm for better form'
            arrow.append([(c_unnorm[12][1], c_unnorm[12][2]), (c_unnorm[16][1], c_unnorm[16][2])])

        else:
            # print('- Arm angle looks good at the contact point')
            feedback['contact_point']['Elbow Angle'] = 'Looks good!'

    return arrow, feedback
def fh_check_follow_through(ft_list, f_unnorm, feedback):
    ...
    # Check right foot position
    if right_foot:
        distance_feet = abs(right_foot[1] - pro_bh[32][1])
        if distance_feet < 80:
            # print('- Right foot is properly positioned')
    ...
    # Check left foot position
    if left_foot:
        distance_feet = abs(left_foot[1] - pro_bh[31][1])
        if distance_feet < 80:
            # print('- Left foot is properly positioned')
    ...
    if right_hip and left_hip:
        distance_hip = abs(right_hip[1] - left_hip[1])
        if distance_hip > 100:
            # print('- Hips are properly aligned')
            feedback['follow_through']['Hips'] = 'Looks good!'
        else:
            # print('- Ensure your hips are aligned towards the target')
    ...
    if left_ankle and left_knee and left_hip:
        ...
        if angle_deg > 160:
            ...
            #print('- Straighten your left leg for better form')
            feedback['follow_through']['Left Leg Angle'] = 'Straighten your left leg for better form'
            arrow.append([(f_unnorm[24][1], f_unnorm[24][2]), (f_unnorm[28][1], f_unnorm[28][2])])

    # Check extension of the toes
    if left_ankle and left_knee and left_toes:
        ...
        if angle_deg > 130:
            ...
            feedback['follow_through']['Toes Extension'] = 'Looks good!'
        else:
            #print('- Keep your left foot on its toes')
            feedback['follow_through']['Toes Extension'] = 'Keep your left foot on its toes'
            arrow.append([(f_unnorm[29][1], f_unnorm[29][2]), (f_unnorm[29][1], int(f_unnorm[29][2] - (standard_height *.1)))])

    return arrow, feedback


def fh_draw_outline(img, arrow):
    if arrow:
        for a in arrow:
